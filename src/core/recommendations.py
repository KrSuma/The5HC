...
def get_improvement_suggestions(assessment_data: Dict[str, Any], client_details: Dict[str, Any]) -> Dict[str, List[str]]:
    ...
    # Check if farmer's carry contributed to low strength score
    if assessment_data.get('farmer_carry_weight', 0) < 20 or assessment_data.get('farmer_carry_distance', 0) < 20:
        if is_older:
    ...
    # Balance suggestions
    if (assessment_data.get('single_leg_balance_right_eyes_open', 0) < 30 or 
        assessment_data.get('single_leg_balance_left_eyes_open', 0) < 30):
    ...
    # Cardio suggestions
    # Convert cardio_score (0-25) to 0-4 scale for comparison
    cardio_score = assessment_data.get('cardio_score', 0)
    if cardio_score <= 12.5:  # Equivalent to score <= 2 on 0-4 scale
        if is_older:
