...
class AssessmentService(BaseService):
    ...
    def _create_test_records(self, assessment: Assessment, data: Dict[str, Any]) -> None:
        ...
        # For now, we'll populate the existing Assessment model fields
        # This will be replaced when we migrate to the new model structure
        
        # Overhead Squat
        if any(key.startswith('overhead_squat_') for key in data.keys()):
            assessment.overhead_squat_score = data.get('overhead_squat_score')
            assessment.overhead_squat_knee_valgus = data.get('overhead_squat_knee_valgus', False)
            assessment.overhead_squat_forward_lean = data.get('overhead_squat_forward_lean', False)
            assessment.overhead_squat_heel_lift = data.get('overhead_squat_heel_lift', False)
            assessment.overhead_squat_arm_drop = data.get('overhead_squat_arm_drop', False)
            assessment.overhead_squat_quality = data.get('overhead_squat_quality')
            assessment.overhead_squat_notes = data.get('overhead_squat_notes', '')
        
        # Push-up Test
        if any(key.startswith('push_up_') for key in data.keys()):
            assessment.push_up_reps = data.get('push_up_reps')
            assessment.push_up_score = data.get('push_up_score')
            assessment.push_up_type = data.get('push_up_type', 'standard')
            assessment.push_up_notes = data.get('push_up_notes', '')
        
        # Continue for other test types...
        # (This will be replaced with individual model creation)
    ...
    def _calculate_individual_scores(self, assessment: Assessment) -> Dict[str, float]:
        ...
        # Don't call assessment.calculate_scores() - that would cause recursion
        # Instead, use the existing scores or calculate them directly
        scores = {
            'overhead_squat_score': assessment.overhead_squat_score or 1,
            'push_up_score': assessment.push_up_score or 1,
            'single_leg_balance_score': assessment.single_leg_balance_score_manual or 1,
            'toe_touch_score': assessment.toe_touch_score or 1,
            'shoulder_mobility_score': assessment.shoulder_mobility_score or 1,
            'farmers_carry_score': assessment.farmer_carry_score or 1,
            'harvard_step_score': assessment.harvard_step_test_score_manual or 1,
        }
    ...
    def _calculate_risk_assessment(self, assessment: Assessment, scores: Dict) -> Tuple[float, Dict]:
        ...
        risk_data.update({
            'overhead_squat_knee_valgus': assessment.overhead_squat_knee_valgus,
            'overhead_squat_forward_lean': assessment.overhead_squat_forward_lean,
            'overhead_squat_heel_lift': assessment.overhead_squat_heel_lift,
        })
    ...
    def get_assessment_statistics(self, assessment: Assessment) -> Dict[str, Any]:
        ...
        stats = {
            'basic_info': {
                ...
                'trainer': assessment.trainer.user.username if assessment.trainer else None,
        ...
        for field_name, test_type in test_mappings.items():
            ...
            if score is not None:
                try:
                    ...
                    # Skip if NormativeData doesn't have get_percentile method
                    pass
    ...
    def calculate_performance_age(self, assessment: Assessment) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # NormativeData model might not have the expected fields
            pass
